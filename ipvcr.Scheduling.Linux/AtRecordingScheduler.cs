...
public class AtRecordingScheduler : ITaskScheduler
{
    ...
    public void ScheduleTask(ScheduledTask task)
    {
        ...
        var startTime = task.StartTime.ToLocalTime().ToString("HH:mm MM/dd/yyyy");
        ...
        var scriptContent = @$"#!/bin/bash" + Environment.NewLine +
            $@"# this script is generated by ipvcr at {DateTimeOffset.UtcNow}" + Environment.NewLine +
    ...
    public IEnumerable<ScheduledTask> FetchScheduledTasks()
    {
        ...
        foreach (var line in lines)
        {
            ...
            var jobOutputLines = jobOutput.Split('\n', StringSplitOptions.RemoveEmptyEntries);
            var taskDefinitionLine = jobOutputLines.FirstOrDefault(l => l.Contains("TASK_DEFINITION"));
            if (string.IsNullOrEmpty(taskDefinitionLine))
            {
                _logger.LogError("No serialized task found for job {jobId}", jobId);
                continue;
            }
            var taskjson = taskDefinitionLine[16..].Trim('\'', '\r', '\n').Replace("\\", string.Empty).Replace("; export TASK_DEFINITION", string.Empty);
